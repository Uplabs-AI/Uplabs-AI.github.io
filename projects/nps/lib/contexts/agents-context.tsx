...
export function AgentsProvider({ children }: { children: React.ReactNode }) {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (savedAgents) {
        try {
          ...
          // Si el array está vacío o no es válido, usar agentes por defecto
          if (Array.isArray(parsedAgents) && parsedAgents.length > 0) {
            setAgents(parsedAgents)
          } else {
            console.log("localStorage tenía array vacío, cargando agentes por defecto...")
            setAgents(defaultAgents)
            localStorage.setItem("nps-vox-agents", JSON.stringify(defaultAgents))
          }
        } catch (error) {
          console.warn("Error parsing agents from localStorage:", error)
          setAgents(defaultAgents)
          localStorage.setItem("nps-vox-agents", JSON.stringify(defaultAgents))
        }
      } else {
        console.log("No hay agentes en localStorage, cargando agentes por defecto...")
        setAgents(defaultAgents)
        localStorage.setItem("nps-vox-agents", JSON.stringify(defaultAgents))
      }
  ...
  // Save agents to localStorage SOLO después de la inicialización
  useEffect(() => {
    if (isInitialized && typeof window !== 'undefined') {
      console.log("Guardando agentes en localStorage:", agents.length, "agentes")
  ...
  const resetToDefaults = () => {
    console.log("Reseteando a agentes por defecto...")
    setAgents(defaultAgents)
    if (typeof window !== 'undefined') {
      localStorage.setItem("nps-vox-agents", JSON.stringify(defaultAgents))
    }
    setIsInitialized(true) // Marcar como inicializado después del reset
  }

  const clearLocalStorage = () => {
    console.log("Limpiando localStorage completamente...")
    if (typeof window !== 'undefined') {
      localStorage.removeItem("nps-vox-agents")
      setAgents(defaultAgents)
      localStorage.setItem("nps-vox-agents", JSON.stringify(defaultAgents))
    }
    setIsInitialized(true)
  ...
  return (
    <AgentsContext.Provider value={{ agents, addAgent, updateAgent, deleteAgent, toggleAgent, resetToDefaults, clearLocalStorage }}>
      {children}
